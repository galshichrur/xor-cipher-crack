...
def guess_keys(ciphertext: list[int], key_size: int) -> Iterator[tuple[str, str]]:
    """
    Brute force all key combinations of given size.
    Yields (key, decrypted ciphertext) for each guess.
    """

    if key_size <= 0:
        raise ValueError('Key size must be greater than 0')

    for key in generate_all_keys(key_size):
        decrypted = XORCipher.encrypt_decrypt_list(ciphertext, key)
